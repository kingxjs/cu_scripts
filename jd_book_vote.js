...
let token = '',LZ_TOKEN_KEY = '',LZ_TOKEN_VALUE = '',Referer = '',RefererOrgin = '',nickname = '';
let Host = '', venderId = ``,shopId = ``,pin =  ``,lz_jdpin_token = ``;
let hotFlag = false;
let attrTouXiang = '',actorUuid = '';
let hotList = [];
let shareList = [];
let skuId = "13038669"//投票书籍id
!(async () => {
    if (!cookiesArr[0]) {
...
//     shareList.push($.shareUuid)
    ...
    let activityList = [{ 'id': 'dzf4facfbeb00111ecac4f02001700', 'endTime': 1650556799000 }];
...
async function main() {
    ...
    shareNick = $.activityData.shareNick;
...
function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'vote':
            ...
            body = `activityId=${activityID}&pin=${encodeURIComponent(pin)}&actorUuid=${actorUuid}&skuId=${skuId}&type=%E7%AB%A5%E4%B9%A6&shareUuid=${$.shareUuid}`;
            break;
        default:
            console.log(`错误${type}`);
    }
    ...
    return new Promise(resolve => {
        $.get(config, (err, resp, data) => {
            ...
            try {
                if (err) {
                    console.info(err)
                    console.log(`getHtml API请求失败，请检查网路重试`)
                } else {
...
function getPostRequest(url, body) {
    const headers = {
        ...
        'Origin' : `https://${Host}`,
        "User-Agent": $.UA,
        'Cookie': `${cookie} LZ_TOKEN_KEY=${LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${LZ_TOKEN_VALUE}; AUTH_C_USER=${pin}; IsvToken=${token}; lz_wq_auth_token=${$.token}; ${lz_jdpin_token}`,
        'Host' : Host,
        'Referer' : Referer,
        'Accept-Language' : `zh-cn`,

...
def query_shop_gift(session: Session, vender_id):
    ...
    try:
        ...
        # 礼物弹窗形式，默认为0，1表示使用内置官方的弹窗形式，2表示活动链接形式
        isvOrofficial = gift_json["isvOrofficial"]
        ...
        if errcode == 0 and gift_list and len(gift_list) > 0:
            if isvActivityLink and isvOrofficial == 2:
                shop_gift_from_activity_link(
                    session, vender_id, isvActivityLink)
            else:
                for gift in gift_list:
                    if gift["prizeType"] == "4":  # 4 为京豆,1 为店铺东券,6 为店铺积分
                        logging.info(
                            "query_shop_gift: get beans {}".format(gift["discount"]))
                        shop_gift_from_official(
                            session, vender_id, gift["activityId"])
            ...
            logging.info(
                "query_shop_gift:{} please re-login...".format(str(gift_json['msg'])))
            # # no login
...
def shop_gift_from_activity_link(session: Session, vender_id, activity_link):
    ...
    if path_part == "https://lzkj-isv.isvjcloud.com/wxShopGift/activity":
        ...
        try:
            ...
            data = "userId=" + user_id + "&token=" + \
                session.cookies.get('wq_auth_token') + "&fromType=WeChat"
            ...
            if content['result']:
                ...
                for gift in gift_list:
                    if gift['type'] is 'jd':
                        logging.info("shop_gift_from_activity_link get beans:{}".format(
                            str(gift['takeNum'])))
        ...
        try:
            ...
            logging.info(
                "shop_gift_from_activity_link getShopGiftActivity: {}".format(str(ret_json)))
            if ret_json['isTake']:
            ...
            logging.debug(
                "shop_gift_from_activity_link getShopGiftPrize: {}".format(str(gift_json)))
            logging.info("shop_gift_from_activity_link getShopGiftPrize: {}".format(
                str(gift_json['data'])))
            return True
        except Exception:
            logging.exception(sys.exc_info())
...
def shop_gift_from_official(session: Session, vender_id, activity_id):
    ...
    try:
        api = "https://wq.jd.com/mjgj/column/DrawShopGift?channel=7&sceneval=2&g_login_type=1&callback=jsonpCBKK&g_ty=ls&venderId=" \
              + vender_id + "&activityId=" + activity_id + \
        ...
        if jsonpCBKK and 'jsonpCBKK' in jsonpCBKK:
            ...
            if gift_json["errcode"] == 0:
                logging.info("shop_gift_from_official: {}".format(
                    str(gift_json['msg'])))
                return True
...
def query_shop_active(session: Session, vender_id):
            ...
            if gift_list and len(gift_list) > 0:
                for gift in gift_list:
                    if gift['state'] == 1:  # 1 为可领取，4 为已领取
                        ...
                        logging.info(
                            "query_shop_active: this gift have been draw...")
                    elif gift['state'] == 0:
                        logging.info("query_shop_active: this gift expired...")
                    else:
                        logging.info(
                            "query_shop_active：{}".format(str(gift_json)))
    ...
    if giftId:
        if jingBean:
            print(f"{vender_id} 有豆豆礼包")
            return True
        else:
            print(f"{vender_id} 没有豆豆礼包")
    else:
        print(f"{vender_id} 没有关注礼包")
...
def shop_active_give_gift(session: Session, vender_id, active_id, gift_id):
    ...
    try:
        api = "https://wq.jd.com/fav_snsgift/GiveShopGift?sceneval=2&g_login_type=1&callback=jsonpCBKP&g_ty=ls&venderId=" \
              + vender_id + "&activeId=" + active_id + "&giftId=" + \
            gift_id + "&_=" + str(int(time.time() * 1000))
        headers = {
            'Host': 'wq.jd.com',
            'Accept': '*/*',
            'Accept-Language': 'zh-CN,zh;q=0.8,en-US;q=0.5,en;q=0.3',
...
def shop_draw_from_activity_link(session: Session, link):
    ...
    if path_part == "https://lzkj-isv.isvjcloud.com/wxShopGift/activity":
        ...
        try:
            ...
            # {"result":false,"data":"pin get error","count":0,"errorMessage":"获取账户转换信息出错，请重新进入一次"}
            logging.info(
                "shop_draw_from_activity_link getMyPing: {}".format(getMyPing))
            ...
            if ret is None or 'data' not in ret:
                logging.error(
                    "shop_draw_from_activity_link: get activityContent error...")
                return
            ...
            while (ret_json['result'] and canDrawTimes > 0):
                ...
                logging.info("shop_draw_from_activity_link: " +
                             session.post(start_api, data=data).text)
        ...
        try:
            ...
            lottery_json = json.loads(session.post(
                getLottery_api, json=payload).text)
            if lottery_json and lottery_json['leftTime'] > 0:
                draw = session.post(draw_api, json=payload).text
                logging.info(
                    "shop_draw_from_activity_link draw: {}".format(str(draw)))
            else:
                logging.info(
                    "shop_draw_from_activity_link: no leftTime to draw...")
        except Exception:
            logging.exception(sys.exc_info())
        ...
        try:
            ...
            data = "venderId=" + venderId + "&wxtoken=" + \
                token + "&yxId=" + yxId + "&actId=1"
            buyerTokenJson = json.loads(session.post(api, data=data).text)
            if buyerTokenJson['drawNum'] > 0:
                buyPin = buyerTokenJson['buyPin']
                logging.info("shop_draw_from_activity_link left drawNum:".format(
                    str(buyerTokenJson['drawNum'])))
                ...
                checkApi = "https://fjzy-isv.isvjcloud.com/index.php?mod=games&action=check&venderId=" + venderId \
                           + "&actId=1&yxId=" + yxId + \
                    "&token=45b7a585-df50-416c-b0be-e9fa5ac2bb4a_1543986226118-jd&buyPin=" + buyPin
                data = "code=" + yxId

                check = session.post(checkApi, data=data).text
                logging.info(
                    "shop_draw_from_activity_link: {}".format(str(check)))
            else:
...
def query_shop_sign_draw(session: Session, vender_id):
    ...
    try:
        ...
        if gift_json["errcode"] != 0:  # errcode : 0 表示成功
            logging.error("query_shop_sign_draw: {}".format(
                str(gift_json['msg'])))
            return
...
def batch_query_shop_fav_gift(session: Session, vender_list):
    ...
    try:
        api = "https://wq.jd.com/mjgj/column/BatchQueryShopFavGift?venderIds=" + str(
            ...
            + "&channel=7&_=" + str(int(time.time() * 1000)) + \
            "&g_login_type=0&callback=jsonpCBKE&g_ty=ls"
        headers = {
            'Host': 'wq.jd.com',
            'Accept': '*/*',
            'Accept-Language': 'zh-CN,zh;q=0.8,en-US;q=0.5,en;q=0.3',
...
def batch_get_shop_info_by_vender_id(session: Session, vender_list):
    ...
    try:
        ...
        if jsonpCBKE is None or 'jsonpCBKE' not in jsonpCBKE:
            logging.error(
                "batch_get_shop_info_by_vender_id: get response error...")
            return

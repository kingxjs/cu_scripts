/*
互助码-上报
cron 3 0 * * * jd_share_sb.js
 */
const $ = new Env("互助码-上报");
                ...
                if (data.data.bizCode === 0) {
                  ...
                  $.taskVos.map((item) => {
                    if (item.taskType === 14) {
                      ...
                      $.ddfactory = item.assistTaskDetailVo.taskToken
                    }
                  });
                }
              }
          ...
          try {
            if (safeGet(data)) {
              ...
              if (oc(() => data.data.result.taskVos)) {
                $.JDHealth = oc(() => data.data.result.taskVos[0].assistTaskDetailVo.taskToken)
                console.log(`【京东账号${$.index}（${$.UserName}）京东健康】${$.JDHealth}`);
                dealCodeByActivity("health", `${$.JDHealth}`)
              }else{
...
async function getShareCode() {
  ...
  var now = new Date()
  await getJDFruit()
  if ($.farmInfo && $.farmInfo.farmUserPro && $.farmInfo.farmUserPro.shareCode) {
    var today = now.getDate()
    var body = `activityId=farm&sharecode=${$.farmInfo.farmUserPro.shareCode}&today=${today}`
    await runTimes(body);
  }
  await $.wait((1 + Math.random()) * 1000)
  await getJdPet()
  if ($.petInfo && $.petInfo.shareCode) {
    var body = `activityId=pet&sharecode=${$.petInfo.shareCode}`
    await runTimes(body);
  }
  await $.wait((1 + Math.random()) * 1000)
  await getPlantBean()
  if ($.myPlantUuid) {
    var body = `activityId=bean&sharecode=${$.myPlantUuid}`
    await runTimes(body);
  }
  await $.wait((1 + Math.random()) * 1000)
  // await getJdFactory()
  // await getJxFactory()
  // if ($.encryptPin) {
  //   var body = `activityId=jxfactory&sharecode=${$.encryptPin}`
  //   await runTimes(body);
  // }
  // await $.wait((1 + Math.random()) * 1000)
  await getSgmh()
  if ($.sgmh) {
    var body = `activityId=sgmh&sharecode=${$.sgmh}`
    await runTimes(body);
  }
  await $.wait((1 + Math.random()) * 1000)
  await getJDHealth()
  if ($.JDHealth) {
    var body = `activityId=health&sharecode=${$.JDHealth}`
    await runTimes(body);
  }
  console.log(`======账号${$.index}结束======\n`)
  await $.wait((1 + Math.random()) * 1000)
}
async function  runTimes(body) {
  return new Promise((resolve) => {
    console.info( `https://api.jdsharecode.xyz/api/runTimes0701?${body}`)
    $.get({
      url: `https://api.jdsharecode.xyz/api/runTimes0701?${body}`
    }, (err, resp, data) => {
      try {
        if (err) {
          console.log("runTimes: API查询请求失败 ‼️‼️");
          console.log(JSON.stringify(err));
          $.logErr(err);
        } else {
          console.log(data)

...
[Script]
cron "0 7 * * *" script-path=zq_Adv_video.js,tag=中青看点
...
let cookieYouth = $.getdata('zq_cookie');
let artBody = $.getdata('zqwzbody');
let readTimes = $.getdata('zq_timebodys');
        ...
        if (!$.isNode() && cookieYouth.indexOf("#") == -1) {
            ...
            timeArr.push(readTimes)
             ...
             if (!$.isNode() && cookieYouth.indexOf("#") > -1) {
                ...
                if (process.env.zq_cookie && process.env.zq_cookie.indexOf('#') > -1) {
                    cookieYouth = process.env.zq_cookie.split('#')
                } else if (process.env.zq_cookie && process.env.zq_cookie.indexOf('\n') > -1) {
                    cookieYouth = process.env.zq_cookie.split('\n')
                } else {
                    cookieYouth = [process.env.zq_cookie]
                };
                if (process.env.zqwzbody && process.env.zqwzbody.indexOf('&') > -1) {
                    artBody = process.env.zqwzbody.split('&')
                } else if (process.env.zqwzbody && process.env.zqwzbody.indexOf('\n') > -1) {
                    artBody = process.env.zqwzbody.split('\n')
                } else {
                    artBody = [process.env.zqwzbody]
                };
                if (process.env.zq_timebodys && process.env.zq_timebodys.indexOf('&') > -1) {
                    readTimes = process.env.zq_timebodys.split('&')
                } else if (process.env.zq_timebodys && process.env.zq_timebodys.indexOf('\n') > -1) {
                    readTimes = process.env.zq_timebodys.split('\n')
                } else {
                    readTimes = [process.env.zq_timebodys]
                }
            };

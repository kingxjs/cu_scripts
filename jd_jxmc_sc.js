...
[task_local]
#京喜牧场
2 0 0 * * * https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxmc.js, tag=京喜牧场, img-url=https://github.com/58xinian/icon/raw/master/jdgc.png, enabled=true

================Loon==============
[Script]
cron "2 0 0 * * *" script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxmc.js,tag=京喜牧场

===============Surge=================
京喜牧场 = type=cron,cronexp="2 0 0 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxmc.js

============小火箭=========
京喜牧场 = type=cron,script-path=https://raw.githubusercontent.com/Aaron-lv/sync/jd_scripts/jd_jxmc.js, cronexpr="2 0 0 * * *", timeout=3600, enable=true
 */
...
//       await $.wait(2000);
  ...
  function uploadShareCode(code) {
    return new Promise(async resolve => {
      ...
      console.info(bean,farm)
      
      $.get({url: `https://api.jdsharecode.xyz/api/autoInsert/jxmc?sharecode=${code}&bean=${bean}&farm=${farm}&pin=${$.UserName}`, timeout: 30 * 1000}, (err, resp, data) => {
        try {
          if (err) {
            console.log(JSON.stringify(err))
  ...
  function  getBeanShareCode() {
    return new Promise(async resolve => {
      $.post(taskPostClientActionUrl(`functionId=plantBeanIndex&body=${encodeURIComponent(
      JSON.stringify({version: "9.0.0.1", "monitor_source": "plant_app_plant_index", "monitor_refer": ""})
    )}&appid=ld&client=apple&area=5_274_49707_49973&build=167283&clientVersion=9.1.0`,`plantBeanIndex`), async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`)
            ...
            if(data && data.data && data.data.jwordShareInfo && data.data.jwordShareInfo.shareUrl){
              data = data.data.jwordShareInfo.shareUrl.split('Uuid=')
              if(data){
                data = data[1]
              }
            }
      ...
      $.post(opt, async (err, resp, data) => {
        try {
          if (err) {
            ...
            data = JSON.parse(data);
            if(data  && data.farmUserPro){
              data = data.farmUserPro.shareCode
            }else{
              data=""
            }
          }

...
$.VAL_cookies = process.env.BD_TB_COOKIE
if(!$.VAL_cookies){
    throw new Error(`请先获取贴吧cookie`)
}
$.CFG_isOrderBars = $.getdata('CFG_tieba_isOrderBars') || 'false' // 1: 经验排序, 2: 连签排序
$.CFG_maxShowBars = $.getdata('CFG_tieba_maxShowBars') * 1 || 50 //每次通知数量
$.CFG_maxSignBars = $.getdata('CFG_tieba_maxSignBars') * 1 || 1 // 每次并发执行多少个任务
$.CFG_signWaitTime = $.getdata('CFG_tieba_signWaitTime') * 1 || 5000 // 每次并发间隔时间 (毫秒)
!(async () => {
  await tieba()
  await zhidao()
  await showmsg()

  await notify.sendNotify(`${$.name}`, $.descAll.join('\n'));
...
function fetch_tbs() {
  return new Promise((resove) => {
    const url = { url: 'http://tieba.baidu.com/dc/common/tbs', headers: { Cookie: $.VAL_cookies } }
    $.get(url, async (err, resp, data) => {
      try {
        const _data = JSON.parse(data)
        // 处理异常
...
async function signbars(bars) {
  ...
  for (let bar of bars.filter((bar) => !bar.isSign)) {
    const signbarAct = (resove) => {
      ...
      //   }
      // })
      const url = { url: 'http://c.tieba.baidu.com/c/c/forum/sign', headers: { Cookie: $.VAL_cookies } }
      let dataMd5 = `kw=${bar.name}tbs=${$.tieba.tbs}tiebaclient!!!`
      url.body = `kw=${bar.name}&tbs=${$.tieba.tbs}&sign=${MD5(dataMd5)}`
      url.headers['Host'] = 'tieba.baidu.com'
      url.headers['User-Agent'] = 'User-Agent: Mozilla/5.0 (iPhone; CPU iPhone OS 13_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.1 Mobile/15E148 Safari/604.1'
      ...
      $.post(url, (err, resp, data) => {
        try {
          const _data = JSON.parse(data)
          bar.iscurSign = true
          bar.issignSuc = _data.error_code === '0' || _data.error_code === '1101'
          bar.signNo = _data.error_code
          if (_data.error_code === '1102')
            bar.signMsg = '签得太快!'
          else if (_data.error_code === '2150040')
            bar.signMsg = '需要验证码!'
          else if (_data.error_code === '160002')
            bar.signMsg = `${bar.name} 已签到！`
          else if (_data.error_code !== '0')
            bar.signMsg = _data.error
        } catch (e) {
          bar.iscurSign = true
          bar.issignSuc = false
          bar.signNo = null
